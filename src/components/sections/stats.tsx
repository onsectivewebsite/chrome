...
function Counter({
  ...
  return (
    <span ref={ref} className="tabular-nums">
      ...
      <span className="text-[#d4b078]">{suffix}</span>
    </span>
  );
}
        ...
        >
          <div>
            ...
            <div className="mt-10 flex items-center gap-4">
              <div className="flex -space-x-2">
                {["#7a0d1c", "#d4b078", "#3a0610", "#b08144"].map((c) => (
                  <span
                    key={c}
                    className="h-8 w-8 rounded-full border-2 border-[#07070b] bg-gradient-to-br shadow-[0_4px_12px_-4px_rgba(0,0,0,0.6)]"
                    style={{ background: `linear-gradient(135deg, ${c}, ${c}80)` }}
                  />
                ))}
              </div>
              <div className="text-[12.5px] text-[#e9e4d8]/55 font-light">
                Trusted by clients from <span className="text-[#ead7af]">60+ countries</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {items.map((s, i) => (
              ...
              >
                <div className="pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-[#d4b078]/8 blur-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                <div className="font-display text-[52px] md:text-[58px] leading-none tracking-[-0.03em] text-[#f6f0e1]">
                  <Counter to={s.k} suffix={s.suffix} />
                </div>
                <div className="mt-4 hairline" />
                <div className="mt-3 text-[13.5px] font-medium text-[#f6f0e1]">
                  {s.label}
                </div>
                <div className="mt-1 text-[11.5px] text-[#8a8395] font-light">{s.sub}</div>
              </motion.div>
            ))}
          </div>
        </motion.div>
